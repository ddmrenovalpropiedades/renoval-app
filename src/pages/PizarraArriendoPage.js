...
export default function PizarraArriendoPage() {
  useAuth();
  ...
  const totals = useMemo(() => {
    let totalCLP = 0;
    filtered.forEach(r => { const { amount, isUF } = parsePrice(r.precio); if (amount) totalCLP += isUF && uf ? amount * uf : isUF ? 0 : amount; });
    return { totalCLP };
  }, [filtered, uf]);
