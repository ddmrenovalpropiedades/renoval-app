...
export default function CalculadoraPage() {
  ...
  const totalDias = diasEnMes(fechaISO);
  const diaLlegada = diaDelMes(fechaISO);
  const proporcional =
  ...
  // Manejo de fecha: acepta escritura libre dd/mm/yyyy y también pegado
  const handleFechaChange = (e) => {
    let val = e.target.value;
    // Solo dígitos y /
    val = val.replace(/[^0-9/]/g, '');
    // Auto-insertar / al escribir
    if (val.length === 2 && fechaInput.length === 1) val = val + '/';
    if (val.length === 5 && fechaInput.length === 4) val = val + '/';
    setFechaInput(val);
