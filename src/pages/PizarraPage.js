...
const ENCARGADO_EMAIL = {
  DD: 'ddm@renovalpropiedades.com', FD: 'fdm@renovalpropiedades.com',
  EA: 'edith@renovalpropiedades.com', FG: 'fernanda@renovalpropiedades.com',
};
...
const formatDateCL = (iso) => { if (!iso) return ''; return new Date(iso + 'T12:00:00').toLocaleDateString('es-CL'); };
...
// ── EMPTY_FORM incluye todos los campos que se comparan ───────
const EMPTY_FORM = {
...
async function createAutoTasks(trigger, propiedad, e1, e2, tipo, fechaEntrega) {
  const { data: templates } = await supabase.from('task_templates').select('*').eq('trigger', trigger).eq('active', true).order('position', { ascending: true });
  ...
  for (const tpl of templates) {
    ...
    if (tpl.condition === 'renovacion_dev_gar') {
      ...
      await supabase.from('tasks').insert({ owner_email: assigneeEmail, title: taskTitle, category: 'Publicar/Arrendar', completed: false, recurrence: 'none', next_occurrence: nextOcc, position: -Date.now(), notes: `Dev Gar programada para ${nextOcc}` });
      continue;
    }
    ...
    for (let i = 0; i < (tpl.subtasks || []).length; i++) {
      await supabase.from('tasks').insert({ owner_email: assigneeEmail, title: tpl.subtasks[i], category, parent_id: parentTask.id, completed: false, position: i });
    }
  }
}
...
function DateCellInput({ value, onChange }) {
  ...
  return (
    <div onClick={() => ref.current && ref.current.showPicker && ref.current.showPicker()} style={{ cursor: 'pointer', fontSize: 11, position: 'relative' }}>
      {value ? formatDateCL(value) : <span style={{ color: '#dadce0' }}>—</span>}
      <input ref={ref} type="date" value={value || ''} onChange={e => onChange(e.target.value)} style={{ position: 'absolute', opacity: 0, pointerEvents: 'none', width: 0, height: 0 }} />
    </div>
  );
}
...
function DateFieldNew({ value, onChange, hasError }) {
  const ref = useRef(null);
  return (
    <div onClick={() => ref.current && ref.current.showPicker && ref.current.showPicker()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: hasError ? '1px solid #ea4335' : '1px solid #dadce0', borderRadius: 6, background: hasError ? '#fce8e6' : '#fff', padding: '0 6px', height: 28, cursor: 'pointer', minWidth: 100, boxSizing: 'border-box', position: 'relative' }}>
      <span style={{ fontSize: 12, color: value ? '#202124' : '#9aa0a6', userSelect: 'none' }}>{value ? formatDateCL(value) : 'dd/mm/aaaa'}</span>
    ...
    <div style={rentStyles.overlay} onClick={e => e.target === e.currentTarget && onCancel()}>
      <div style={rentStyles.modal}>
        ...
        <div style={rentStyles.field}>
          ...
          <div onClick={() => ref.current && ref.current.showPicker && ref.current.showPicker()} style={{ ...rentStyles.input, cursor: 'pointer', display: 'flex', alignItems: 'center', position: 'relative', color: entrega ? '#202124' : '#9aa0a6' }}>
            {entrega ? formatDateCL(entrega) : 'Seleccionar fecha...'}
            <input ref={ref} type="date" value={entrega} onChange={e => setEntrega(e.target.value)} style={{ position: 'absolute', opacity: 0, pointerEvents: 'none', width: 0, height: 0 }} />
          </div>
        </div>
        {hasPromo && (
          <div style={rentStyles.field}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
              {['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'].map(m => {
                ...
                return <button key={m} type="button" onClick={() => { const list = meses.split(',').map(s=>s.trim()).filter(Boolean); setMeses((selected ? list.filter(x=>x!==m) : [...list,m]).join(', ')); }} style={{ padding: '4px 10px', borderRadius: 20, fontSize: 12, cursor: 'pointer', border: selected ? '1px solid #1a73e8' : '1px solid #dadce0', background: selected ? '#e8f0fe' : '#fff', color: selected ? '#1a73e8' : '#5f6368', fontWeight: selected ? 700 : 400 }}>{m}</button>;
              })}
            </div>
          </div>
        )}
        <div style={rentStyles.actions}>
          <button onClick={() => comision && entrega && onConfirm({ comision, entrega, meses: hasPromo ? meses : '' })} disabled={!comision || !entrega} style={{ ...rentStyles.confirmBtn, ...(!comision || !entrega ? { background: '#e8eaed', color: '#9aa0a6', cursor: 'not-allowed' } : {}) }}>Confirmar arriendo</button>
...
// ── FORM_KEYS para comparación de cambios ─────────────────────
...
// ── PropertyRow ───────────────────────────────────────────────
// La solución para URL: onUrlSaved callback que actualiza form local directamente
function PropertyRow({ row, onSave, onDelete, onRented, isNew=false, onCancelNew, uf, onOpenUrlModal, onOpenDisponibilidad }) {
  ...
  // ── Clave del fix: abrir modal pasando form (no row), y recibir
  //    callback onUrlSaved que actualiza form local directamente ──
  ...
  if (isNew) return (
    <tr style={{ background: '#f0f7ff', borderBottom: '1px solid #e8eaed' }}>
      <td style={styles.tdProp}><PropertyAutocomplete value={form.propiedad} onChange={v => setForm(p => ({ ...p, propiedad: v }))} onSelect={handlePropSelect} hasError={!!errors.propiedad} inputStyle={{ height: 28, boxSizing: 'border-box', fontSize: 12 }} /></td>
      ...
      <td style={styles.tdCenter}><SlashInputNew value={form.db||''} onChange={v=>setForm(p=>({...p,db:v}))} /></td>
  ...
  return (
    <tr style={{ background: '#fff' }} onMouseEnter={e => e.currentTarget.style.background = '#f8f9fa'} onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
      ...
      <td style={styles.tdActions}>
        ...
        {confirmDelete ? (
          <><button onClick={async () => { await onDelete(row.id); }} style={styles.actionBtnRed}><Trash2 size={13} /></button><button onClick={() => setConfirmDelete(false)} style={styles.actionBtnGray}><X size={12} /></button></>
        ) : (
...
export default function PizarraPage() {
  ...
  // ── Fix URL: guardar en Supabase y llamar callback _onSaved
  //    para actualizar form local de la fila sin pasar por row ──
          ...
          <div style={styles.filters}>
            {ENCARGADOS_ALL.map(e => (
              <button key={e} onClick={() => setFilterE(prev => prev.includes(e) ? prev.filter(x=>x!==e) : [...prev,e])}
                style={{ ...styles.filterBtn, ...(filterE.includes(e) ? { background: `${ENCARGADO_COLORS[e]}22`, color: ENCARGADO_COLORS[e], borderColor: ENCARGADO_COLORS[e], fontWeight: 700 } : {}) }}>{e}</button>
            ))}
